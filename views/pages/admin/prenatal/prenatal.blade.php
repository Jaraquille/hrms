<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="bg-white rounded-lg p-10">
            <h1 class="uppercase font-bold mb-4">Pre-natal visit</h1>
            <hr>
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        @foreach ($patients as $patient)
                            <tr class="{{ $loop->iteration % 2 != 0 ? 'bg-white' : 'bg-gray-50' }}">
                                ...
                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <a href="prenatal-form/{{$patient->id}}" class="text-indigo-600 hover:text-indigo-900 uppercase">select</a>
                                </td>
                             </tr>
                        @endforeach
                ...
                {{-- display success --}}
                  ...
                  <div class="overflow-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg mt-10">
                      <div class="p-2">
                          <h1 class="font-bold">Prenatal Records</h1>
                      ...
                      <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="bg-white">
                          ...
                          @foreach ($prenatals as $prenatal)
                              <tr class="{{ $loop->iteration % 2 != 0 ? 'bg-white' : 'bg-gray-50' }}">
                                  <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ $loop->iteration }}</td>
                                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ ucfirst($prenatal->patient->name) }}</td>
                                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ \Carbon\Carbon::parse($prenatal->created_at)->format('l, d F Y H:i:s A') }}</td>
                                  {{-- <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{$patient->age}}</td> --}}
                                  {{-- <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 uppercase">{{substr($patient->gender, 0, 1)}}</td> --}}
                                  {{-- <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ ucfirst($prenatal->type_of_patient) }}</td> --}}
                                  {{-- <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ ucfirst($prenatal->method) }}</td>
                                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ ucfirst($prenatal->reason) }}</td> --}}
                                  {{-- <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"> --}}
                                      {{-- @foreach ($prenatal->assessments as $assessment)
                                  ...
                                  {{-- </td> --}}
                                  <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <a href="prenatal-view/{{$prenatal->id}}" class="mr-2 text-indigo-600 hover:text-indigo-900">
                                      {{-- fontawesome view icon --}}
                                      <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="prenatal-edit/{{$prenatal->id}}" class="mr-2 text-blue-600 hover:text-blue-900">
                                      {{-- fontawesome edit icon --}}
                                      <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="prenatal-delete/{{$prenatal->id}}" class="text-red-600 hover:text-red-900">
                                      {{-- fontawesome edit icon --}}
      ...
      <script>
          const dataPie = {
              ...
              datasets: [
              {
                  label: "Patients",
                  data: [
                      "{{$patient_count['employed']}}", 
                      "{{$patient_count['unemployed']}}",
                      "{{$patient_count['student']}}",
                  ],
                  backgroundColor: [
                  "#FFE600",
                  "#007aff",
