...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200">
            ...
            <div class="text-xs text-slate-500 italic">{{ (Auth::user()->user_type == 4) ? 'Nurse' : ((Auth::user()->user_type == 3) ? "BHW" : ((Auth::user()->user_type == 2) ? "Midwife" : "Patient")) }}</div>
        </div>
        <ul>
            <li>
                <a class="font-medium text-sm text-indigo-500 hover:text-indigo-600 flex items-center py-1 px-3" href="{{ route('profile.show') }}" @click="open = false" @focus="open = true" @focusout="open = false">Account Settings</a>
            </li>
            <li>
